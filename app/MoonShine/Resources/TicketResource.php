...
class TicketResource extends ModelResource
{
    ...
    protected function formFields(): iterable
    {
        return [
            Box::make([
                ...
                Date::make(__('moonshine::ui.fields.anwered_at'), 'anwered_at')
                    ->format('Y-m-d H:i')
                    ->withTime(),
                Text::make(__('moonshine::ui.fields.title'), 'title')->disabled(),
                Textarea::make(__('moonshine::ui.fields.description'), 'description')->disabled(),
            ])
        ];
    }
    ...
    protected function detailFields(): iterable
    {
        return [
            ...
            Date::make(__('moonshine::ui.fields.created_at'), 'created_at')->format('Y-m-d H:i'),
        ];
    }
    ...
    protected function rules(mixed $item): array
    {
        return [
            ...
            'anwered_at'        => ['nullable', Rule::date()->beforeOrEqual(now()->format('Y-m-d H:i'))],
        ];
    }
    ...
    {
        return [
            DateRange::make(__('moonshine::ui.fields.created_at'), 'created_at')
                ->format('Y-m-d H:i')->nullable(),
            BelongsTo::make(__('moonshine::ui.fields.ticket_status'), 'ticketStatus', fn(TicketStatus $ticketStatus) => $ticketStatus->name, TicketStatusResource::class)
                ->searchable()
                ->nullable(),
            Email::make(__('moonshine::ui.fields.email'), 'customer.email')
                ->onApply(fn(Builder $builder, ?string $value) => $builder->whereHas('customer', function (Builder $query) use ($value) {
                    $query->where('email', 'like', "%{$value}%");
                }))
                ->nullable(),
            Text::make(__('moonshine::ui.fields.phone_number'), 'phone_number')
                ->onApply(fn(Builder $builder, ?string $value) => $builder->whereHas('customer', function (Builder $query) use ($value) {
